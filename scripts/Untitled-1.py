...
def run_pipeline(event_id: int,
    ...
    # Market data is now used for context only — not as price anchor
    if market:
        print(f"   Market avg recent sale: ${market['avg_order_size']:.2f}")
        print(f"   Days before event:      {market['days_before_event']}")
    
    # Use listed_price if provided, otherwise fall back to MinPrice
    if listed_price:
    ...
    # Days before event — always calculate from actual event date
    # Market data days_before_event = days from SALE date, not today
    try:
        ...
        # Fallback to market data if date parse fails
